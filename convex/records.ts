...
export const addProtein = mutation({
  args: {
    ...
    totalFiber: v.optional(v.number()),
  },
  handler: async (ctx, args) => {
    const userId = await getAuthUserId(ctx);
    if (!userId) throw new Error("Not authenticated");
    ...
    if (existing) {
      ...
      await ctx.db.patch(existing._id, { 
        ...
        totalKcal: (existing.totalKcal ?? 0) + (args.totalKcal ?? 0),
        totalCarbs: (existing.totalCarbs ?? 0) + (args.totalCarbs ?? 0),
        totalSugars: (existing.totalSugars ?? 0) + (args.totalSugars ?? 0),
        totalFiber: (existing.totalFiber ?? 0) + (args.totalFiber ?? 0),
        items: mergedItems,
      });
      ...
      await ctx.db.insert("proteinRecords", {
        ...
        totalKcal: args.totalKcal ?? 0,
        totalCarbs: args.totalCarbs ?? 0,
        totalSugars: args.totalSugars ?? 0,
        totalFiber: args.totalFiber ?? 0,
        items: args.items,
      });
...
export const deleteProteinRecord = mutation({
  args: { id: v.id("proteinRecords") },
  handler: async (ctx, args) => {
    const userId = await getAuthUserId(ctx);
    if (!userId) throw new Error("Not authenticated");
...
export const updateProteinRecord = mutation({
  args: {
    id: v.id("proteinRecords"),
    items: v.array(v.object({
      id: v.string(),
      name: v.string(),
      count: v.number(),
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.patch(args.id, {
      items: args.items,
      totalGrams: args.totalGrams,
      totalKcal: args.totalKcal,
      totalCarbs: args.totalCarbs,
      totalSugars: args.totalSugars ?? 0,
      totalFiber: args.totalFiber,
    });
  },
});
