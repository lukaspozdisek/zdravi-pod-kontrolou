...
async function requireAdminOrManager(ctx: any) {
  ...
  console.log("academy.requireAdminOrManager userId:", userId);
  ...
  console.log("academy.requireAdminOrManager isManager:", isManager);
...
export const createCategory = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Get highest order
    const categories = await ctx.db.query("academyCategories").collect();
    ...
    const id = await ctx.db.insert("academyCategories", {
      name: args.name,
...
export const deleteCategory = mutation({
  args: { id: v.id("academyCategories") },
  handler: async (ctx, args) => {
    await requireAdminOrManager(ctx);
...
export const reorderCategories = mutation({
  args: {
    categoryIds: v.array(v.id("academyCategories")),
  },
  handler: async (ctx, args) => {
    await requireAdminOrManager(ctx);
...
export const getTopics = query({
  args: { categoryId: v.id("academyCategories") },
  handler: async (ctx, args) => {
    const topics = await ctx.db
      .query("academyTopics")
...
export const createTopic = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const now = Date.now();
    const id = await ctx.db.insert("academyTopics", {
...
export const getArticles = query({
  args: { topicId: v.id("academyTopics") },
  handler: async (ctx, args) => {
    const articles = await ctx.db
      .query("academyArticles")
...
export const createArticle = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const now = Date.now();
    const id = await ctx.db.insert("academyArticles", {
...
export const getFullAcademyData = query({
  args: {},
  handler: async (ctx) => {
    const categories = await ctx.db
      .query("academyCategories")
