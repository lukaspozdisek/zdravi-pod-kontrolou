...
// Admin email that cannot have role changed
const SUPER_ADMIN_EMAIL = "lukas.pozdisek@gmail.com";

// Set user role (admin only)
export const setUserRole = mutation({
  args: { 
    userId: v.id("users"),
    role: v.union(v.literal("admin"), v.literal("manager"), v.literal("moderator"), v.literal("user")),
  },
  handler: async (ctx, { userId, role }) => {
    ...
    // Protect super admin email from role changes
    if (user.email === SUPER_ADMIN_EMAIL) {
      return { ok: false, code: "FORBIDDEN", message: "Nelze změnit roli hlavního administrátora" };
    }
...
// Get app settings (public - for profile tab)
  ...
  handler: async (ctx) => {
    ...
    if (!settings) {
      return {
        allowUSMode: true,
        allowPeptides: true,
        allowRetatrutide: true,
      };
    }
    
    return {
      allowUSMode: settings.allowUSMode,
      allowPeptides: settings.allowPeptides,
      allowRetatrutide: settings.allowRetatrutide,
    };
  },
});

// Update user app settings (admin only, per-user)
export const updateUserAppSettings = mutation({
  args: {
    userId: v.id("users"),
    adminAllowUSMode: v.optional(v.boolean()),
    adminAllowPeptides: v.optional(v.boolean()),
    adminAllowRetatrutide: v.optional(v.boolean()),
  },
  handler: async (ctx, args) => {
    ...
    await ctx.db.patch(args.userId, {
      ...(args.adminAllowUSMode !== undefined && { adminAllowUSMode: args.adminAllowUSMode }),
      ...(args.adminAllowPeptides !== undefined && { adminAllowPeptides: args.adminAllowPeptides }),
      ...(args.adminAllowRetatrutide !== undefined && { adminAllowRetatrutide: args.adminAllowRetatrutide }),
    });
