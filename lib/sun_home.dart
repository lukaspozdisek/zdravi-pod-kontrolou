...
class SunHome extends StatefulWidget {
  ...
  @override
  State<SunHome> createState() => _SunHomeState();
}

class _SunHomeState extends State<SunHome> {
  int sectionIndex = 1;
  int pillIndex = 0;
  ...
  Widget build(BuildContext context) {
    ...
    final muted = Theme.of(context).textTheme.bodySmall?.color ?? Colors.white70;

    final bg = SunTheme.backgroundGradient(widget.genderMode, brightness);

    // Demo data (jen pro vizuál)
    final kcalLine = <double>[120, 180, 220, 260, 420, 520, 610, 740, 820, 910, 980];
    final weeklyKcal = <double>[1400, 1550, 1200, 1700, 1500, 1320, 980];
    final macros = _MacroData(protein: 78, carbs: 65, fat: 42); // fat jen pro vizuál
      ...
      body: Container(
        ...
        child: SafeArea(
          child: Padding(
            ...
            child: ListView(
              children: [
                Text('Good afternoon, Lukas!', style: Theme.of(context).textTheme.headlineSmall),
                const SizedBox(height: 6),
                Text('Your day at a glance', style: Theme.of(context).textTheme.bodySmall),
...
SunSectionCarousel(
  items: const [
    ...
    SunSectionMenuItem(label: 'Diary', icon: Icons.book_rounded),
    SunSectionMenuItem(label: 'Food', icon: Icons.restaurant_rounded),
    SunSectionMenuItem(label: 'Body', icon: Icons.fitness_center_rounded),
    SunSectionMenuItem(label: 'Chat', icon: Icons.forum_rounded),
  ],
  index: sectionIndex,
  gender: widget.genderMode,
...
const SizedBox(height: 12),
_RowGenderSwitch(
  mode: widget.genderMode,
  accent: accent,
  onChanged: widget.onToggleGender,
),
                ...
                _GlassCard(
                  child: Column(
                    ...
                    children: [
                        ...
                        children: [
                          ...
                          Expanded(
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                _StatLine(label: 'Protein', value: '78 g', color: accent, muted: muted),
                                _StatLine(label: 'Carbs', value: '65 g', color: secondary, muted: muted),
                                _StatLine(label: 'Fiber', value: '22 g', color: SunColors.success, muted: muted),
                                const SizedBox(height: 10),
                                Wrap(
                                  spacing: 8,
                                  runSpacing: 8,
                                  children: [
                                    _Chip(text: 'Water 4/8', icon: Icons.water_drop, color: secondary),
                                    _Chip(text: 'Injection in 2d', icon: Icons.vaccines, color: accent),
                                    _Chip(text: 'Mood: Good', icon: Icons.emoji_emotions, color: accent),
                                  ],
                                ),
                              ],
                            ),
                      ...
                      const SizedBox(height: 12),
                      Text('Energy today', style: Theme.of(context).textTheme.titleMedium),
                      const SizedBox(height: 10),
                      ...
                      gender: widget.genderMode,
                      brightness: brightness,
                      gradient: SunTheme.brandGradient70_30(widget.genderMode),
                      onPressed: () {},
                    ),
                          ),
                          const SizedBox(width: 10),
                          _SquareAction(icon: Icons.qr_code_scanner, label: 'Scan'),
                        ],
                      ),
                    ],
                  ),
                ...
                _GlassCard(
                  child: Column(
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          Expanded(child: _MiniMetric(label: 'Protein', value: '${macros.protein}g', color: accent)),
                          const SizedBox(width: 8),
                          Expanded(child: _MiniMetric(label: 'Carbs', value: '${macros.carbs}g', color: secondary)),
                          const SizedBox(width: 8),
                          Expanded(child: _MiniMetric(label: 'Fat', value: '${macros.fat}g', color: SunColors.warning)),
                        ],
                      ),
                    ],
                  ),
                ...
                _GlassCard(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 10),
                      Text('Goal: 1500 kcal/day', style: Theme.of(context).textTheme.bodySmall),
                    ],
                  ),
                ),
                ...
                _GlassCard(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('How do you feel today?', style: Theme.of(context).textTheme.titleMedium),
                      const SizedBox(height: 10),
                      Wrap(
                        spacing: 10,
                        runSpacing: 10,
...
class _GlassCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: isDark ? Colors.white.withOpacity(0.06) : Colors.white.withOpacity(0.92),
        borderRadius: BorderRadius.circular(20),
        border: Border.all(color: isDark ? Colors.white10 : const Color(0xFFE9EBF3)),
        boxShadow: [
          BoxShadow(
      ...
      child: CustomPaint(
        ...
        child: Center(
          child: Column(
            ...
            children: [
              Text(labelTop, style: TextStyle(fontSize: 26, fontWeight: FontWeight.w700, color: textColor)),
              Text(labelBottom, style: TextStyle(fontSize: 12, color: mutedColor)),
            ],
          ),
        ),
      ),
...
class _StatLine extends StatelessWidget {
  ...
  const _StatLine({required this.label, required this.value, required this.color, required this.muted});
  ...
  Widget build(BuildContext context) {
    return CustomPaint(
      ...
      child: Align(
        ...
        child: Text('09:00  →  21:00', style: TextStyle(fontSize: 11, color: textColor)),
      ),
    );
  }
}
...
class _MacroData {
  ...
  final int carbs;
  final int fat;
  const _MacroData({required this.protein, required this.carbs, required this.fat});
}
...
class _StackedMacroBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return CustomPaint(
      ...
      child: Align(
        alignment: Alignment.bottomLeft,
        child: Text('Protein / Carbs / Fat', style: TextStyle(fontSize: 11, color: textColor)),
      ),
    );
  }
}
...
class _StackedMacroBarPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    double x = 0;
    canvas.drawRRect(
      RRect.fromRectAndRadius(Rect.fromLTWH(x, y, pW, barHeight), const Radius.circular(14)),
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(Rect.fromLTWH(x, y, fW, barHeight), const Radius.circular(14)),
...
class _WeeklyBars extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return CustomPaint(
      ...
      child: Align(
        alignment: Alignment.bottomLeft,
        child: Text('Mon  Tue  Wed  Thu  Fri  Sat  Sun', style: TextStyle(fontSize: 11, color: textColor)),
      ),
    );
  }
}
